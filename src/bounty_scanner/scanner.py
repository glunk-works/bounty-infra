...
def run_recon_pipeline(
    domain: str, severities: str = "medium,high,critical", timeout: int = 1800
):
        ...
        if os.path.getsize(artifacts.subs_file) > 0:
            ...
            if os.path.getsize(artifacts.live_file) > 0:
                ...
                with (
                    ...
                    subprocess.run(
                        ["nuclei", "-s", severities, "-jsonl", "-silent"],
                        stdin=f_in,
                        stdout=f_out,
                        check=True,
                        timeout=timeout,
...
def main():
    ...
    with run_recon_pipeline(
        args.domain, severities=args.severities, timeout=args.timeout
    ) as artifacts:
