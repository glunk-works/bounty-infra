...
SEVERITY_WEIGHT = {"critical": 4, "high": 3, "medium": 2, "low": 1, "info": 0, "unknown": 0}
...
@contextmanager
def run_recon_pipeline(domain: str, severities: str = "medium,high,critical", timeout: int = 1800):
    """Runs recon pipeline using disk-backed I/O. Filters severities at the Nuclei engine level."""
    findings = []
    ...
    try:
        ...
        with open(artifacts.subs_file, 'w') as f_out:
            subprocess.run(["subfinder", "-d", domain, "-silent"], stdout=f_out, check=True, timeout=timeout)
        ...
        if os.path.getsize(artifacts.subs_file) > 0:
            ...
            with open(artifacts.subs_file, 'r') as f_in, open(artifacts.live_file, 'w') as f_out:
                subprocess.run(["httpx", "-silent"], stdin=f_in, stdout=f_out, check=True, timeout=timeout)

            if os.path.getsize(artifacts.live_file) > 0:
                logger.info(f"Running nuclei scanning for severities: {severities}...")
                with open(artifacts.live_file, 'r') as f_in, open(artifacts.nuclei_file, 'w') as f_out:
                    # NUCLEI LEVEL FILTERING: Using the -s flag to filter severities at the engine level
                    subprocess.run(["nuclei", "-s", severities, "-jsonl", "-silent"], stdin=f_in, stdout=f_out, check=True, timeout=timeout)
...
def triage_findings(findings: List[dict], target_severities: set, max_findings: int) -> Optional[TriageReport]:
    ...
    actionable_findings.sort(
        key=lambda x: SEVERITY_WEIGHT.get(x.get("info", {}).get("severity", "unknown").lower(), 0), 
        reverse=True
    )
    ...
    if total_actionable > max_findings:
        logger.warning(f"Truncated LLM payload from {total_actionable} down to {max_findings} items.")
        truncation_warning = (
...
def upload_to_s3(domain: str, report: Optional[TriageReport], artifacts: ReconArtifacts):
    ...
    try:
        ...
        if report:
             s3.put_object(Bucket=bucket_name, Key=f"{base_prefix}/triage_report.json", Body=report.model_dump_json(indent=2))
        s3.put_object(Bucket=bucket_name, Key=f"{base_prefix}/raw_findings.json", Body=json.dumps(artifacts.findings, indent=2))
        
        # Upload the raw disk files into the artifacts subfolder
        if os.path.getsize(artifacts.subs_file) > 0:
            s3.upload_file(artifacts.subs_file, bucket_name, f"{artifacts_prefix}/subdomains.txt")
        
        if os.path.getsize(artifacts.live_file) > 0:
            s3.upload_file(artifacts.live_file, bucket_name, f"{artifacts_prefix}/live_hosts.txt")
            
        if os.path.getsize(artifacts.nuclei_file) > 0:
            s3.upload_file(artifacts.nuclei_file, bucket_name, f"{artifacts_prefix}/nuclei_output.jsonl")
...
def main():
    parser = argparse.ArgumentParser(description="Agentic Vulnerability Scanning Pipeline")
    ...
    parser.add_argument("--severities", type=str, default="medium,high,critical", help="Comma-separated severities for Nuclei and AI triage (default: medium,high,critical)")
    parser.add_argument("--max-findings", type=int, default=50, help="Max findings to send to the LLM (default: 50)")
    ...
    with run_recon_pipeline(args.domain, severities=args.severities, timeout=args.timeout) as artifacts:
        logger.info(f"Found {len(artifacts.findings)} raw findings. Triaging...")
        report = triage_findings(artifacts.findings, target_severities, args.max_findings)
